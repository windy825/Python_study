...
for i in range(3, n+3):
    for j in range(3, m+3):             # 별도의 설명이 필요없는 완전 낮은 수준 그냥 모든 모양을 찾는 완전 탐색
...
def dfs(a, b, cnt, total):
    ...
    if total + (4-cnt) * maxx <= answer:   # 최악의 경우를 피하기 위한 가지치기, 유망성 검사 (다익스트라 파생 개념)
        return
...
```python
def check(i):
    if not i//10:  # 1자리일때 조건, (아래의 자릿수 탐색만 있을시 오류가 나더군요, 따라서 추가한 조건임)
